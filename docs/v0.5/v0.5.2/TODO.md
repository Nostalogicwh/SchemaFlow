...
- [x] 增加前端 UI 提示，显示当前工作流是否已缓存登录凭证

**实现细节**：
- 新增 `checkCredentialStatus()` 函数，检查 localStorage 中是否缓存了凭证
- 在 Header 工作流名称旁显示"已登录"标签（蓝色），hover 显示提示信息
- 选择工作流时自动检查凭证状态
...
- `frontend/src/App.tsx` — 凭证状态检测和 UI 显示
...
**修复方案**：
- [x] 保留 `HybridElementLocator` 中 CSS 失效 → AI 后备的链路
- [x] AI 定位成功后，将选中的 CSS 选择器通过 WS 消息（新增 `selector_update` 类型）回传给前端
- [x] 前端收到 `selector_update` 后，自动更新对应节点的 `selector` 配置字段
- [x] `click` 和 `input_text` action 使用 `locate_element` 并返回 `effective_selector`

**实现细节**：
1. **后端修改** (`backend/engine/actions/browser.py`)：
   - `click_action` 和 `input_text_action` 使用 `locate_element()` 函数
   - 返回 `effective_selector`（AI 定位成功后生成的选择器）

2. **后端修改** (`backend/engine/executor.py`)：
   - 节点执行后检查返回值是否包含 `effective_selector`
   - 如果与原始选择器不同，发送 `selector_update` WebSocket 消息

3. **前端修改** (`frontend/src/types/workflow.ts`)：
   - 添加 `selector_update` 到 `WSMessageType`
   - 新增 `WSSelectorUpdate` 接口

4. **前端修改** (`frontend/src/hooks/useWebSocket.ts`)：
   - 添加 `onSelectorUpdate` 回调选项
   - 在 `handleMessage` 中处理 `selector_update` 消息

5. **前端修改** (`frontend/src/App.tsx`)：
   - 添加 `handleUpdateNodeConfig()` 函数，更新工作流节点配置
   - 传递 `onSelectorUpdate` 回调给 `useWebSocket`

**涉及文件**：
- `backend/engine/ai/locator.py` — 混合定位逻辑
- `backend/engine/actions/browser.py` — click/input_text 使用 locate_element
- `backend/engine/executor.py` — 发送选择器更新消息
- `frontend/src/types/workflow.ts` — 新增消息类型
- `frontend/src/hooks/useWebSocket.ts` — 处理 selector_update
- `frontend/src/App.tsx` — 更新节点配置
...
**修复方案**：
- [x] 检查各处 `border` 样式，统一为 `border-gray-200`
...
**根因**：`BaseNode.tsx` 中节点容器设置了 `overflow-hidden`，导致 source handle（`!right-[-7px]` 向外偏移）被裁剪。
...
**建议**：
- ✅ P0 阻断性问题已全部完成
- ✅ OPT-02、OPT-03、OPT-04、OPT-06、OPT-07 简单优化已完成
