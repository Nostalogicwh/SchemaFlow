# V0.4 可执行计划 - 前端样式体系重构 + 功能问题修复

## 目标

1. **修复核心功能问题**（6个）：输入法冲突、按钮交互、功能重复、AI定位、登录状态
2. **建立统一的前端 UI 组件体系**：解决样式散乱、组件重复、视觉不一致
3. **搭建测试框架基础**

## 需要解决的功能问题

详见 `存在问题.md`，本计划将以下问题纳入实施：

| # | 问题 | 影响范围 | 解决方案 |
|---|------|----------|----------|
| 1 | 输入法与回车键重叠 | NodePanel表单 | 输入框优化，回车事件处理 |
| 2 | 重试按钮无交互效果 | ExecutionPanel | Button组件loading状态 |
| 3 | 运行和执行功能重复 | Header | 统一执行入口 |
| 4 | 创建工作流弹窗丑陋 | WorkflowList | Modal组件重构 |
| 5 | AI定位目标执行失败 | AI编排功能 | 定位逻辑修复 |
| 6 | 浏览器登录状态丢失 | BrowserManager | 上下文复用优化 |
...
#### 目标
合并为一个统一的"执行"入口，根据当前模式自动选择执行方式。

#### 涉及文件
- `frontend/src/components/Header.tsx` — 移除重复按钮，统一入口

#### 实施步骤
1. 分析两个按钮的当前行为差异
2. 保留一个主要执行按钮
3. 根据当前模式（RPA/Agent）自动路由到相应执行逻辑
4. 更新按钮状态和提示文本
...
#### 现状
跳转页面后需要重新登录，BrowserManager没有正确复用上下文状态。

#### 目标
确保在同一工作流执行期间，浏览器上下文和页面状态得到正确保持。

#### 涉及文件
- `backend/engine/browser_manager.py` — 修复上下文复用逻辑
- `backend/engine/context.py` — 确保状态正确传递

#### 实施步骤
1. 检查BrowserManager的`_reused_page`和`_is_cdp`状态管理
2. 确保工作流执行期间使用同一个browser context
3. 验证cookies和localStorage的持久化
4. 测试跨页面跳转的场景
...
#### 现状
AI编排时元素定位经常失败，可能是定位策略或等待逻辑问题。

#### 目标
提高AI编排中元素定位的成功率和稳定性。

#### 涉及文件
- `backend/engine/nodes/ai_nodes.py` — 检查AI节点实现
- `backend/engine/browser_manager.py` — 检查定位辅助方法

#### 实施步骤
1. 分析当前定位失败的具体场景和错误日志
2. 添加智能等待机制（waitForSelector等）
3. 实现多重定位策略回退（ID → CSS选择器 → XPath → 文本）
4. 添加定位失败时的截图和调试信息
5. 测试常见网站的定位场景
...
#### 现状
在NodePanel的输入框中，使用输入法时按回车会触发表单提交，而不是完成输入法选词。

#### 目标
正确处理输入法组合事件，避免误触发。

#### 涉及文件
- `frontend/src/components/FlowEditor/panels/NodePanel.tsx` — 修复键盘事件处理

#### 实施步骤
1. 检查当前表单的事件监听
2. 添加`isComposing`检查，忽略输入法组合期间的回车事件
3. 测试中文、日文等输入法的场景
4. 确保正常回车提交仍然可用
...
#### 现状
重试按钮点击后没有loading状态或视觉反馈。

#### 目标
为ExecutionPanel的所有操作按钮添加统一的loading状态。

#### 涉及文件
- `frontend/src/components/ExecutionPanel/index.tsx` — 添加按钮状态管理
- `frontend/src/components/ui/Button.tsx` — 确保loading状态支持

#### 实施步骤
1. 在ExecutionPanel中添加按钮loading状态管理
2. 使用新的Button组件替换现有按钮
3. 确保异步操作期间按钮显示loading并禁用
4. 测试网络延迟场景下的体验
...
#### 现状
创建工作流的弹窗样式简陋，没有统一的Modal设计。

#### 目标
使用新的Modal组件重构，提供一致的用户体验。

#### 涉及文件
- `frontend/src/components/WorkflowList/index.tsx` — 重构创建弹窗
- `frontend/src/components/ui/Modal.tsx` — 使用新组件

#### 实施步骤
1. 等待P3阶段Modal组件完成
2. 重构创建工作流弹窗，使用新的Modal组件
3. 统一表单样式和验证提示
4. 添加创建成功的反馈
...
| 阶段 | 任务 | 依赖 | 说明 | 对应问题 |
|------|------|------|------|----------|
| **F0** | 问题3：统一执行功能入口 | 无 | Header中"运行"和"执行"合并为一个入口 | #3 |
| **F1** | 问题6：浏览器上下文复用 | 无 | 修复BrowserManager状态管理，保持登录态 | #6 |
| **F2** | 问题5：AI定位目标修复 | 无 | 修复AI编排中的元素定位逻辑 | #5 |
| **F3** | 问题1：输入法与回车冲突 | F0 | NodePanel表单输入优化 | #1 |
| **F4** | 问题2：按钮交互效果 | P1 | ExecutionPanel重试按钮loading状态 | #2 |
| **F5** | 问题4：创建工作流弹窗 | P3 | 使用新Modal组件重构 | #4 |
...
| **P4** | C1 Header 重构 + C2 侧边栏优化 | P1, P2, F0 | 使用新组件重构，统一执行入口 |
| **P5** | C3 节点样式升级 + C5 节点属性面板 | P2, P3, F3 | 编辑器核心区域，修复输入法问题 |
| **P6** | C4 执行面板优化 | P1, P3, F2, F4 | 执行监控区域，修复AI定位+按钮交互 |
| **P7** | D1 加载状态 + D2 空状态 + D3 动效统一 | P1 | 交互增强 |
| **P8** | E1-E3 无障碍 | P1-P6 完成后 | 全局扫描修复 |
| **P9** | E4 测试框架 + UI 组件测试 | P1-P3 完成后 | 可与 P4-P7 并行 |
| **P10** | E5 后端优化 | F1 完成后 | 可独立进行 |

### 并行策略

1. **F0-F2** 可并行执行（功能修复互不依赖）
2. **P0-P3** 可并行执行（基础组件开发）
3. **F3-F5** 依赖基础组件完成后进行
4. **P4-P7** 需要功能修复和基础组件都完成后进行
